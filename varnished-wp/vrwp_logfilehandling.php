...
add_action( 'admin_notices', 'vrwp_check_create_log_folder' );

// create log folder
// return true if folder has been created or exists else false
function vrwp_createLogFolder() {
	$error=false;
	if (!@is_dir (VRWP_LOGPATH)) {
		if (!@mkdir (VRWP_LOGPATH, 0777)) {
			$vrwp_on_off=0;
			update_option('vrwp_on_off', 0);
			$error=true;
		}
	}

	# check if log folder is writeable
	if (!@is_writable(VRWP_LOGPATH) ) {

		# trying to set permissions
		if (!@chmod(VRWP_LOGPATH, 0777)) {
			$vrwp_on_off=0;
			update_option('vrwp_on_off', 0);
			$error=true;
		}
		...
		$emptyFile=VRWP_LOGPATH.'index.html';
...
# delete log folder
function vrwp_deleteLogFolder() {
	# delete log folder and logs
	if (@is_dir(VRWP_LOGPATH)) {
		vrwp_deltree(VRWP_LOGPATH);
	}
}

// try to create log folder and print success or error on admin panel
function vrwp_check_create_log_folder() {
	# check for folder
	if (vrwp_check_folder_error()) {
		if (!vrwp_createLogFolder()) {
			print '<div id="message" class="error">'.__("Empty Plugin Template (EPT) Error: Can't write to log folder ", EMU2_I18N_DOMAIN).VRWP_LOGPATH.__(" Permissions 777 needed.", EMU2_I18N_DOMAIN).'</div>';
		} else {
			print '<div id="message" class="updated">'.__("Empty Plugin Template (EPT): Log folder created: ", EMU2_I18N_DOMAIN).VRWP_LOGPATH.'</div>';
		}
	}
}

function vrwp_check_folder_error() {
	$error=false;
	# check if log folder is writeable
	if (!@is_writable(VRWP_LOGPATH) ) {
		# trying to set permissions
...
function vrwp_writelog() {
	...
	$logFile=VRWP_LOGPATH.'/ops-'.date("Y-m").".log";
